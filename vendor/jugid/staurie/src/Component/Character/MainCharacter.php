...
class MainCharacter extends AbstractComponent {
    ...
    private function fight(string $target_name) : void {
        ...
        $pp->writeLn("Fight starts against $target_name !", 'green');
        $round = 1;
        while($target_health > 0 && $player_health > 0) {
            ...
            switch($fight_choice) {
                case '0':
                    ...
                    $pp->writeLn("$target_name attacks. You loose $target_damages", 'red');
                    $pp->writeLn("$target_name looses $player_damages", 'red');
                    break;
                case '1':
            ...
            $pp->writeUnder("You : $player_health, $target_name: $target_health");
            $round++;
        }

        if($target_health <= 0) {
            $pp->writeLn("$target_name is dead. Get ready for the next one !");
